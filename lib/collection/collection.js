...
_.extend(collection.prototype, {
	...
	_prepareObject: function(args) {
		if(args instanceof this._template) {
			...
			if(!args.hash) args.hash = args.primaryKey ? this.hash(args.get(args.primaryKey)) : this.hash(args.id);
			return args;
		}
	...
	get: function(args) {
		...
		return this._byId[args.id] || this._byId[args.hash] || this._byId[args];
	},
	set: function(args, options) {
		...
		if(toAdd.length > 0) {
			toAdd.forEach(function(item) {
				...
				if(item.primaryKey) {
					this._byId[item.get(item.primaryKey)] = item;
				} else {
